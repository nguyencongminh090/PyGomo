...
class Engine(IEngine):
    ...
    def receive(self, option):
        ...
        match option:
            ...
            case 'output':
                # For ABOUT, START 15
                output = self.__stdoutReader.getOutput(self.__setting.TimeManage.timeLeft / 1000)
                return output.lower() if output is not None else ''

    def __getMove(self):
        output = self.__stdoutReader.getCoord(self.__setting.TimeManage.timeLeft / 1000)
        message = self.__stdoutReader.getMessage()
        if output is None:
            return TimeOut(f"Engine didn't return move after {self.__setting.TimeManage.timeLeft}ms")
...
class Controller:
    ...
    def isReady(self, boardSize=15):
        ...
        return self.__engine.receive('output')  == 'ok'
...
class Move:
    ...
    def __infoClassify(self):
        ...
        infoDict = {'depth': self.__info[self.__info.index('depth') + 1],
                    'ev'   : Evaluation(int(self.__info[self.__info.index('ev') + 1])),
...
class Evaluation:
    ...
    def toWinrate(self):
        return (math.e ** (self.ev / 200)) / (1 + math.e ** (self.ev / 200)) * 100
